...
- **共 10 个仓库**：8 个项目仓库 + `.github`（组织门面）+ `website`（文档站 + 元资产）。
- **需要门面，但不需要单独的"门面仓库"**：门面职责拆给 `.github`（组织首页）和 `website`（读者入口），不再建第三个 meta 仓库——仓库越少，"从组织首页到任一项目 ≤2 次点击"越容易保证。
...
├── website            # 组织门面 B：MkDocs 文档站 + roadmap/designs（元资产）
...
- GitHub Pages 挂在此仓库，站点即组织对外主 URL（`system-in-miniature.github.io` 或自定义域名）。
...
## 发布顺序（依赖当前未 commit 状态）

1. 用户审完八仓库工作树 → 各仓库首次正式 commit（修复 + 配套 + 双语一并入库，或按仓库拆分，用户定）。
2. 建组织 + `.github` + `website`（迁入 docshub 与 portfolio 元资产，改造 sync 脚本为 CI 模式）。
3. 各项目仓库补标准件（LICENSE/CI/badges/topics）→ push → 打 v0.1.0。
4. website 首次 Pages 部署，验证全站链接。
5. 组织首页 pin + profile README 上线。

## 已确认决策

1. **审查报告不迁入 website**：审查报告是独立资产，另行发布。
2. **`portfolio/` 本地目录的去向**：迁入 website 仓库后本地 sim-workspace 保留 symlink 工作区形态不变；`portfolio/site/`（旧手写站）删除。
